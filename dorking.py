...
class ZeroScann():
    ...
    def scan(self):
        ...
        parser = argparse.ArgumentParser(prog="dorking.py", description="Tools AnarchoXploit Shell Scanner")
        parser.add_argument("-u", dest="domain", help="url yang akan di scan")
        parser.add_argument("-w", dest="wordlist", help="wordlist kamu")
        args = parser.parse_args()
        if not args.domain:
            sys.exit("\033[36musage: python dorking.py -u example.com -w wordlist.txt")
        if not args.wordlist:
            sys.exit("\033[36musage: python dorking.py -u example.com -w wordlist.txt")
            
        # handle url website format
        site = args.domain
        print("\033[96m[?] \033[0mMulai!!!...")
        print("\033[96m[!] \033[0mSemoga Beruntung","\n")
        time.sleep(3)
        if not site.startswith("http://"):
            site = "http://"+site+"/admin/webroot/cache/"
    ...
    def banner():
        ...
        info = """\033[33m
 # =============================================== #
 # Dorking Shell Timthumb 1.xx                     #
 # Name    : Ramdhani1337                          #\033[0m
 # Github  : https://github.com/Ramdhani1337       #
 # web     : https://www.atisdale.com              #
 # coded by: Marshall                              #
 # =============================================== #
              """
        return info
